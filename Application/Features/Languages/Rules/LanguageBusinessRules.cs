...
namespace Application.Features.Languages.Rules
{
    public class LanguageBusinessRules
    {
        ...
        /*
        public async Task LanguageNameCanNotBeEmptyWhenInserted(string  name)
        {
            if (name=="") throw new BusinessException("Language name cannot be empty");
        }
        */ 
        public async Task LanguageShouldExistWhenRequested(Language? language)
        {
            Language? result = await _languageRepository.GetAsync(b => b.Id == language.Id);

            if (result == null) throw new BusinessException("Requested language does not exist.");
        }
    }
}
